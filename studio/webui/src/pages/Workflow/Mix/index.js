...
import { Button, Modal, ConfigProvider, Radio, Dropdown, Menu } from 'antd';
import { CheckCircleFilled, LoadingOutlined, CaretDownOutlined } from '@ant-design/icons';
...
class Mix extends Component {
  ...
  state = {
    ...
    branchTaskVisible: false // 分批构建弹窗
  };
   ...
   * 点击执行任务
   */
  onImplementClick = async (runType = 'now') => {
  ...
  // 关闭分批构建弹窗
  closebranchTaskModal = () => {
    this.setState({ branchTaskVisible: false });
  };

  render() {
    const { graphId, ontoData, ontologyId } = this.props;
    const {
      entity,
      showIndex,
      isError,
    ...
    return (
      <div className="mix">
            ...
            overlay={
              <Menu>
                <Menu.Item
                  key="branch"
                  onClick={() => {
                    this.onImplementClick('branchtask');
                    this.setState({ branchTaskVisible: true });
                  }}
                >
                ...
                {/* rabbitmq无定时任务 */}
                {!isImmediately && (
                  <Menu.Item key="timeTask" onClick={() => this.setState({ timedTaskVisible: true })}>
                    <span> {intl.get('workflow.conflation.timedrun')}</span>
                  </Menu.Item>
                )}
                <Menu.Item key="now" onClick={() => this.onImplementClick()}>
        ...
        <ModalBranchTask
          ...
          handleCancel={this.closebranchTaskModal}
          graphId={graphId}
          goToTask={this.goToTask}
          ontoId={ontoData?.[0]?.id || ontologyId}
        />
