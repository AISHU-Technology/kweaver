...
const Workflow = props => {
  ...
  const next = res => {
    if (res) {
      ...
      // 权限错误
      if (curCode === 500403 || curCode === 'Studio.SoftAuth.UnknownServiceRecordError') {
  ...
  const handleSave = async () => {
    ...
    if (current === 2 && step3Data?.groupBody) {
      servicesSubGraph.subgraphEdit(graphId, step3Data.groupBody);
    }
    ...
    const body = {
      ...
      graph_otl: step3Data?.ontoBody || ontoData,
