...
export const handleGraphToSummary: HandleRepeatMark = graph => {
  ...
  _.forEach(graph.nodes, (node, index) => {
    const { uid, name = '', alias = '' } = node;
    const lowName = name.toLowerCase();
    const lowAlias = alias.toLowerCase();
    nodesMap[uid] = node;

    if (!nameReg.test(name) || name.length > maxLength || nodesMap[lowAlias] || nodesMap[lowAlias]) {
      nodeIds.push(uid);
  ...
  _.forEach(graph.edges, (edge, index) => {
    const { uid, name = '', startId, endId } = edge;
    ...
    if (!nameReg.test(name) || name.length > maxLength || edgesMap[edgeKey]) {
      edgeIds.push(uid);
