...
const Property = props => {
  const { page, property, checkData, PAGESIZE } = props;
  ...
  const showProperty = property.filter((item, index) => index >= startIndex && index <= endIndex);
          ...
          return (
            ...
            >
              {page === 1 && index === 0 ? (
                ...
                <MinusCircleFilled className="delete" onClick={() => deleteProperty(startIndex + index)} />
              )}
              <Input
                ref={inputRef}
                ...
                autoComplete="off"
                value={item[0]}
                disabled={canInput(page, index)}
                onChange={e => changeInput(e.target.value, startIndex + index, index)}
              ...
              <Select
                ...
                onChange={value => changeSelect(value, startIndex + index)}
              >
                {_.map(LIST, item => (
              ...
              <Switch
                ...
                disabled={page === 1 && index === 0}
                checked={item[2]}
                onClick={value => changeIndex(value, startIndex + index)}
              />
