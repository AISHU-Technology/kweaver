 ...
 * @param nodes 实体点
 */
const filterClass = (nodes: any[]) => {
  const nodeObj = nodes.reduce((res, item) => {
    const { tag, color } = item;
    return res[tag] ? res : { ...res, [tag]: { tag, color } };
  }, {});

  return Object.values(nodeObj);
};

export { handlePathData, filterClass };
