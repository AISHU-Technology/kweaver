...
const SavedConfig: React.ForwardRefRenderFunction<unknown, SavedConfigProps> = (
  ...
  const [configList, setConfigList] = useState<any[]>([]); // 所有配置策略数据
   ...
   * @param obj.id // 知识网络id
   * @param obj.page // 页码
   * @param objquery // 搜索关键字
   * @param needLoading // 是否需要loading动画
   */
  const getConfigs: Function = async (
    { id, page = 1, query = '' }: { id: number; page?: number; query?: string },
    needLoading = true
  ) => {
  ...
  const onCheckConfig = async (item: Record<string, any>) => {
    ...
    if (configRes) {
      ...
      if (graphRes) {
        const graphData = convertData(graphRes.df[0]);
