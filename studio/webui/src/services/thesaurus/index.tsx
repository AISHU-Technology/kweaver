...
const thesaurusCreate = async (data: any) => {
  return await apiService.axiosPost(API.thesaurusCreate, { type: 'file', ...data }, { timeout: 30000 });
}
...
const thesaurusExport = async (data: any) => {
  // return await apiService.axiosPost(API.thesaurusExport, data, { responseType: 'blob' });
      ...
      fileReader.onload = function () {
        ...
        if (msg?.ErrorCode) {
          ERROR_CODE[msg?.ErrorCode]
            ? message.error(intl.get(ERROR_CODE[msg?.ErrorCode])) : message.error(msg?.Description);
        }
      }
      return false;
    }
...
// 词库内导入词汇接口
const thesaurusImportWords = async (data: any) => await apiService.axiosPost(API.thesaurusImportWords, { type: 'file', ...data }, { timeout: 300000 });
...
// 词云图
const wordCloud = async (data: any) => await apiService.axiosGetData(API.wordCloud, data);

// 点击慈云图搜索
const searchWordCloud = async (data: any) => await apiService.axiosGetData(API.searchWordCloud, data);
