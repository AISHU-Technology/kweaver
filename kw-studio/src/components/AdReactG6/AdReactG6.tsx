...
import { generateG6GraphDataByOntologyData } from '@/components/AdReactG6/utils';
import AdG6ToolBar from './AdG6ToolBar/AdG6ToolBar';
 ...
 */
const AdReactG6 = forwardRef<AdReactG6RefProps, AdReactG6Props>((props, ref) => {
  ...
  const graph = useRef<Graph | null>(null); // Instance of G6
  const [g6Init, setG6Init] = useState<boolean>(false); // G6 has been initialized
  const [graphData, setGraphData, getGraphData] = useLatestState<ReactG6GraphDataProps>({ nodes: [], edges: [] });
  const [selectedNode, setSelectedNode, getSelectedNode] = useLatestState<ReactG6NodeDataProps[]>([]);
  const [selectedEdge, setSelectedEdge, getSelectedEdge] = useLatestState<ReactG6EdgeDataProps[]>([]);
  ...
  const initG6GraphConfig = () => {
    registerItemHoverBehavior('hover-item'); // Register node/edge hover event
   ...
   * @param evt
   */
  const handleChangeDataComplete = (evt: G6GraphEvent) => {
    setTimeout(() => {
  ...
  const highlightEdge = (edgeData: ReactG6EdgeDataProps) => {
    ...
    graph.current!.setItemState(edgeId, '_active', true); // highlight edge
  };
  ...
  return (
    <AdResizeObserver onResize={g6GraphResize}>
      <div style={style} className={classNames(prefixCls, className)}>
        ...
        {toolVisible && <AdG6ToolBar style={{ bottom: 20, right: 24 }} graph={graph.current!} />}
      </div>
    </AdResizeObserver>
  );
});

export default AdReactG6;
