...
export type AdKnowledgeNetIconProps = {
  className?: string;
  style?: React.CSSProperties;
  type: string; // icon
  size?: number; // icon size
  fontSize?: number; // icon fontSize
  border?: boolean; // Whether to display borders
};

export const KnowledgeNetIconList = [
  {
...
const AdKnowledgeNetIcon: React.FC<AdKnowledgeNetIconProps> = props => {
  const { className, style = {}, type, size = 20, fontSize = 12, border = true } = props;

  const knowledgeNetIcon = useMemo(() => {
    const target = KnowledgeNetIconList.find(item => item.icon === type);
    if (target) {
      return target;
    }
    return {
...
export default AdKnowledgeNetIcon;
