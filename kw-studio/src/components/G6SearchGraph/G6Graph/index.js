...
class G6Graph extends Component {
  ...
  group = ''; // Group of selected nodes

  state = {
    informationVisible: false, // In and out edge selection bar
    inAndOutVisible: false, // In and out edge expansion bar
    loadingFull: false, // Loading data full screen loading
    selectEdge: '', // Select the edge to be expanded
    visible: false, // Control analysis pop-up window
    anylysisTitle: '',
    inOrOut: 'in', // In and out edge
    modalVisible: false, // Clear with one click
  ...
  init = () => {
    ...
    this.graph.on('aftercreateedge', async e => {
      const { nodes, selectGraph, setEndNode, setStartNode, direction, setPathList, setIsExplorePath, pathType } =
        this.props;
      if (!parseInt(selectGraph.kg_id)) return;
      ...
      try {
        ...
        if (res?.res?.ErrorCode) {
          ...
          return message.error(res?.Description || res?.res?.Description);
        }

        if (res && res.res?.paths) {
          let data = res.res?.paths;
          setPathList({ data: [], count: res.res?.paths.length });
          this.setState({
            pathAllData: data
          });
          return;
  ...
  openToolPanel = e => {
    ...
    addToolPanel(this.group, e.item.getModel().data, this.props.kwLang);
  };

  /**
   * @description 添加数据
   */
  addNodes = (nodes, edges, selected = false) => {
  ...
  outSideSelect = node => {
    ...
    addToolPanel(this.group, node, this.props.kwLang);
  };
    ...
    return (
      <div className="G6-graph-show" ref={this.ref} style={{ backgroundImage: `url(${gridBg})` }}>
        ...
        <div className={loadingFull ? 'top-tip-box' : 'hidden'}>
          ...
          <span className="kw-ml-2">{intl.get('searchGraph.loadingTip')}</span>
        </div>
