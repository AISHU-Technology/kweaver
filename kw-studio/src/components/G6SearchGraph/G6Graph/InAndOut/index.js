...
class InAndOut extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.selectEdge.edge_class !== this.props.selectEdge.edge_class) {
  ...
  initData = ({ page = this.state.page }) => {
    ...
    this.getEdgeData({
      ...
      typeClass: selectEdge.edge_class,
      io: selectEdge.type,
      rid: selectedNode.id,
      page,
      size: PAGESIZE,
    ...
    _.forEach(data, item => {
      ...
      if (selectEdge.type === 'in') {
        _.forEach(item.out_e, outItem => {
          ...
          if (!outItem.name || outItem.name === '_null_') {
            outItem.name = outItem.alias;
          }
        });
      } else {
        _.forEach(item.in_e, inItem => {
          ...
          if (!inItem.name || inItem.name === '_null_') {
            inItem.name = inItem.alias;
          }
        });
      }
      const showName = !name || name === '_null_' ? alias : name;
      item.data = { id, color, name: showName, expand, analysis, alias, properties, class: item.class };
      item.label = showName;
      item.style = {
  ...
  checkedAll = e => {
    ...
    if (e.target.checked) {
      ...
      openEdges = openEdges.filter(item => {
        // 删除被删除的点的所有进出边
        const isDelete = deleteNodeIds.includes(item?.source) || deleteNodeIds.includes(item?.target);
        return !deleteEdgeIds.includes(item.id) && !isDelete;
      });
          ...
          <div>
            {sourceData.length > 0 ? (
              ...
              <div className="inOut-empty">
                <Empty style={{ marginTop: 100 }} description={`${intl.get('global.noResult2')}`} image={kong} />
              </div>
            )}
          </div>
        )}
