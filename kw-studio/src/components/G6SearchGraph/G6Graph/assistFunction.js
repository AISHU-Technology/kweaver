...
const setInfoPosition = (selectedNode, graph, inOrOut) => {
  ...
  if (infoPosition && selectedNode) {
    // Coordinate system conversion for dom positioning
...
const getExpandHandleData = (data, selectedNode, nodes, edges) => {
  // Encountering no color problem
  const defaultColor = 'rgba(0,0,0,0)';
  // Expand edges
  let openEdges = [...edges];

  // Edge id that already exists in the canvas
  const openIds = openEdges.map(item => {
    return item.id;
  });

  let currentData = []; // save node data
  ...
  _.forEach(data, item => {
    ...
    if (item.in_e) {
      _.forEach(item.in_e, inItem => {
        ...
        // Determine whether this edge already exists in the canvas
...
const addCurveOffset = (openEdges, value) => {
  ...
  for (let i = 0; i < openEdges.length; i++) {
    if (openEdges[i].source === addEdge.target && openEdges[i].target === addEdge.source) {
      ...
      // The position of the edge in the array opposite to the arrow direction of the edge, which facilitates positioning of elements when deselecting
...
const getNodesFromEdges = (data, nodes) => {
  // Use hash to store existing nodes
...
const drawMark = (graph, id, isDelete = false, size = 16) => {
  ...
  // Already exists, no need to redraw or delete
