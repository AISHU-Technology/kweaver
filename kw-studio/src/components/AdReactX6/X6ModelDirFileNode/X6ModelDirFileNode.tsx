...
import { Button, Spin, Tooltip, Dropdown, message } from 'antd';
...
const X6ModelDirFileNode = (props: { node: Node; graph: Graph }) => {
  ...
  const refreshData = async () => {
    ...
    const { res, Cause } = (await servicesCreateEntity.getChildrenFile(params)) || {};
    setLoading(false);
    if (Cause) {
   ...
   */
  const viewBtn = async () => {
    // Get complete information about the data source
      ...
      <div className={classNames(prefixCls, 'kw-align-center kw-p-2 kw-border kw-bg-white')}>
        ...
        >
          <FileTree
            ...
            source={modalProps.dataSource} // Selected data source
            errors={{}} // Does the file have any errors? Format：{file id: error msg}，Used to display error icons
            multiple={false}
