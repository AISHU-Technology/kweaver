...
import { Button, Input, Tabs, ConfigProvider, message } from 'antd';
...
import { handleResultName } from './assistFunction';
...
export interface CognitiveSearchProps {
  knData: Record<string, any>; // 知识网络数据
}
...
const CognitiveSearch: React.FC<CognitiveSearchProps> = ({ knData }) => {
  ...
  const [isDrawer, setIsDrawer] = useState(false); // 右侧抽屉

  useEffect(() => {
    const lMain = document.getElementsByClassName('l-main')?.[0]?.parentElement?.parentElement;
    lMain?.addEventListener('click', (e: any) => {
      setIsDrawer(false);
    });
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      configRes?.length && dispatchState({ isNotConfig: false }); // 判断配置
      graphRes?.length && dispatchState({ isNotGraph: false }); // 判断图谱
  ...
  const refreshConfig = () => {
    ...
    configRef.current?.getGraphList(knData.id);
  };
   ...
   * @param page 页码
   */
  const getRes = async ({ ids, query = '', page = 1 }: Record<string, any>) => {
   ...
   * @param query 搜索关键字
   * @param page 页码
   * @param data 测试数据, 包含图谱id和配置
   */
  const getTestRes = async ({ query = '', page = 1, data }: Record<string, any>) => {
    ...
    const signId = ++requestId;
    const startTime: any = new Date();
    const res = await servicesSearchConfig.advSearchTestV2(params);
    ...
    const transCode = ErrorDetails ? (ErrorDetails[0].err.includes('not exist') ? 'EngineServer.ErrKGIDErr' : '') : '';
    const endTime: any = new Date();
    const time = (endTime - startTime) / 1000;
    setResData({ number: 0, time: time.toFixed(2) });
  ...
  const updateTestUrl = (params?: Record<string, any>) => {
    ...
    history.push(`${window.location.pathname}?id=${id}${test}`);
  };
  ...
  const notGraphCallback = (isEmpty: boolean, isClear?: boolean) => {
    ...
    !isEmpty && dispatchState({ isNotGraph: false });
    ...
    <div className="kg-cognitive-search">
      ...
      >
        ...
        <div className="search-ui">
          {isHideRightUI() && (
            <div className="empty-content kw-c-subtext" style={{ paddingTop: 224 }}>
          ...
          <div className={`search-header ${isHideRightUI() && 'hide'}`}>
            <Input
              ...
              onChange={e => {
                dispatchState({ inputValue: e.target.value });
                setIsDrawer(false);
              }}
            />
          ...
          <div className={`result-main ${isHideRightUI() && 'hide'}`}>
            ...
            {(!resData.number || !resData.res) && !selfState.loading && (
              <div className="empty-content kw-c-subtext" style={{ paddingTop: 140 }}>
                <img src={resData.res ? noResult : configSearchTip} alt="empty" />
                {resData.res ? (
                  !selfState.inputValue ? (
                    <p className="tip">{intl.get('searchConfig.inputSearch')}</p>
                  ) : (
