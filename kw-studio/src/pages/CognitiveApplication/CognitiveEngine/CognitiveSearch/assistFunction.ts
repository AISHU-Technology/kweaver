...
const convertData = (origin: Record<string, any>): TGraphData => {
  ...
  const nodeObj = v.reduce((res: any, item: any) => {
    const { name, alias, colour, icon } = item;
 ...
 * @param sort_rules 排序权重
 */

const generateConfig = (selectedData: TConfigData, max_depth: number, sort_rules: TSortRule) => {
  const { nodeScope, nodeRes, edgeScope, nodeAllScope } = selectedData;

  return {
    max_depth,
    start_range: { vertexes: { open: nodeScope } },
    search_range: {
      vertexes: { open: nodeAllScope || [...nodeScope, ...nodeRes] },
...
/**
 * [DATA-311252] 本体删除name后导致搜索出name为空, 当name为空时, 显示class
 * @param resData 后端返回的搜索结果
 */
const handleResultName = (resData: any) => {
  if (!resData?.res?.search) return;
  _.forEach(resData.res.search, (item: any) => {
    item.name = item.tag;

    _.forEach(item.search_path.vertexes, node => {
      node.name = node.tag;
    });

    _.forEach(item.search_path.edges, edge => {
      edge.name = edge.tag;
    });
  });
};

export { cutName, convertData, initConfig, generateConfig, handleProperties, handleResultName };
