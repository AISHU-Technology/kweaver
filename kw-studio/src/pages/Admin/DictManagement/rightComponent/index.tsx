...
import servicesEventStats, { getDictListItemDataType, deleteDictItemDataType } from '@/services/eventStats';
...
const DictManagerRightComp = (props: DictManagerRightCompProps) => {
  ...
  useEffect(() => {
    if (leftInfo) {
      fetchTableData(paginationCurrent, searchInputRef.current?.state.value);
    }
  }, [paginationCurrent]);
  ...
  return leftInfo?.id ? (
    <div className="admin-dictManagement-rightComponent-content">
      ...
      <ADTable
        ...
        locale={{
          emptyText: searchInputRef.current?.state.value ? (
