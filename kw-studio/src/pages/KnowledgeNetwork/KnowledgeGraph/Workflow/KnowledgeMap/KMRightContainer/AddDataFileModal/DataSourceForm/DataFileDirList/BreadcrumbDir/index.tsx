...
interface BreadcrumbDirProps {
  ...
  onLoadData?: (selectedDir: BreadcrumbDataProps) => void;
  onFileSelected?: (selectedFile: BreadcrumbDataProps) => void;
}

export interface BreadcrumbDirRef {
 ...
 * @constructor
 */
const BreadcrumbDir = forwardRef<BreadcrumbDirRef, BreadcrumbDirProps>((props, ref) => {
  ...
  const flatData = useRef<BreadcrumbDataProps[]>([]);
  ...
  const [breadcrumbProps, setBreadcrumbProps, getBreadcrumbProps] = useLatestState({
    dataSource: [] as BreadcrumbDataProps[],
    activeData: undefined as BreadcrumbDataProps | undefined,

    goBackRecord: [] as BreadcrumbDataProps[],
    goAheadRecord: [] as BreadcrumbDataProps[]
  });
      ...
      <div className={classNames(prefixCls, className)} style={style}>
        ...
        <div className={`${prefixCls}-content`}>
          {breadcrumbProps.activeData?.children && breadcrumbProps.activeData?.children.length > 0
            ...
            : !loading && <NoDataBox type="NO_CONTENT" />}
        </div>
      </div>
    </>
  );
...
export default memo(BreadcrumbDir);
