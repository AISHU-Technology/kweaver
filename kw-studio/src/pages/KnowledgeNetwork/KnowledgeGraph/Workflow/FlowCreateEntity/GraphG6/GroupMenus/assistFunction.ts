 ...
 */
export const initGraphGroup = (graph: GraphData, groupList: GraphGroupItem[]) => {
 ...
 * @param graph 图数据
 */
export const mergeBrushToGraph = (brushData: BrushData, graph: GraphData): GraphData => {
  const { targetGroup, nodes, edges } = brushData;
  if (!targetGroup?.id || !(nodes.length + edges.length)) return graph;
 ...
 * @param group 原分组数据
 */
export const autoUpdateGroup = (graph: GraphData, group: GraphGroupItem[]) => {
 ...
 */
export const resetGroups = (group: any[], oldGroup: any[] = []) => {
