...
const SELECT_STROKE_COLOR = 'rgba(0,0,0, 0.1)'; // 选中边框颜色
...
const updateNodeStyle = (graph: TGraph, nodeShape: INode, options?: StylesOptions) => {
  ...
  const stroke = isReset || !isSelected ? '#fff' : '#000';
  const lineWidth = isReset || !isSelected ? 3 : 2;
  graph.updateItem(nodeShape, {
    labelCfg: { style: { opacity } },
    style: { opacity, stroke, lineWidth },
...
const updateEdgeStyle = (graph: TGraph, edgeShape: IEdge, options: StylesOptions) => {
  ...
  graph.updateItem(edgeShape, {
    ...
    style: { endArrow, opacity, stroke: color },
    stateStyles: { selected: { stroke: color } }
  });
};
 ...
 * 节点选中阴影
 */
export const addShadowByNode = (graph: TGraph, id: string, isDelete = false) => {
