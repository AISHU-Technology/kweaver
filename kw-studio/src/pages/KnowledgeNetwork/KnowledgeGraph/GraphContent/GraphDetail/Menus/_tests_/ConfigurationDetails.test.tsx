...
describe('ConfigurationDetails', () => {
  it('test render', () => {
    ...
    expect(headerText.includes(intl.get('graphDetail.knGraphClassDetail'))).toBe(true);
  });
  it('selectedData has value', () => {
    const props = { selectedData: { type: 'entity' }, graphid: {} };
    const wrapper = init(props);
    expect(wrapper.find('.nodeInfo').hasClass('kw-pb-3')).toBe(true);
  });
  it('selectedData has no value', () => {
    const props = { selectedData: {}, graphid: {} };
    const wrapper = init(props);
    const emptyText = wrapper.find('.configurationDetailsRoot-empty').text();
