...
import { generateGroupBody } from '../assistant';
...
export type FlowFooterRef = {
  next: () => void;
  save: () => void;
};

const Footer = forwardRef<FlowFooterRef, FooterProps>((props, ref) => {
  const {
    prev,
    next,
    graphId,
  ...
  const [loading, setLoading, getLoading] = useLatestState(false);
  ...
  const onNext = async () => {
    ...
    if (viewMode) {
      if (graphData.nodes.length === 0) {
        message.error({
          content: [intl.get('createEntity.noEntityToast')],
          className: 'custom-class',
          style: {
            marginTop: '6vh'
          }
  ...
  return (
    <div className="work-flow-footer">
      <ConfigProvider autoInsertSpaceInButton={false}>
        ...
        <Button type={viewMode ? 'primary' : 'default'} className="btn" onClick={onNext}>
