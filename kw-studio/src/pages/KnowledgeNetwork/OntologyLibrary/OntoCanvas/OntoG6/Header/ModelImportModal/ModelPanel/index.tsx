...
const OntoModelPanel: React.ForwardRefRenderFunction<unknown, ModelPanelProps> = (props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    getGraph: () => (selected ? createModelGraph(graphData, selected, canvasRef.current?.graph) : null)
  }));
  ...
  const getModelGraph = async (model: string) => {
    ...
    const graph = parseModelGraph(res, true, extendProperty);
    setGraphData(graph);
  };
  ...
  return (
    <div className="flow-3-model-Panel kw-h-100">
      <div className="p-title">
        <Format.Title style={{ fontWeight: 400 }} level={21}>
          {intl.get('createEntity.selectModel')}
        </Format.Title>
      </div>

      <Select
        className="kw-w-100 kw-mb-4"
      ...
      <div className="preview-box">
        ...
        <div className={classnames('p-canvas kw-w-100 kw-h-100', { 'hide-view': viewType !== 'canvas' })}>
          <PreviewCanvas ref={canvasRef} graphData={graphData} />
        </div>
      </div>
    </div>
  );
...
export default forwardRef(OntoModelPanel);
