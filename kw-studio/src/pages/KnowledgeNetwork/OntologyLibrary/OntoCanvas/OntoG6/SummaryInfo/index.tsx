...
const OntoSummaryInfo = (props: SummaryInfoProps) => {
  const { graph, groupList, onClose, onDelete, onUpdateGraphData, setSelectedElement, onCreateGroup, ontoLibType } =
    props;
  ...
  const [errorMap, setErrorMap] = useState<ErrorMap>({ nodes: [], edges: [] });
  ...
  useEffect(() => {
    ...
    const matchData = _.filter(origin, item => fuzzyMatch(value, item.alias) || fuzzyMatch(value, item.name));
  ...
  return (
    <div className="right-menus-summary-root" style={{ width: 400 }}>
...
export default OntoSummaryInfo;
