...
import { Button, Input, Form, Select, message, ConfigProvider } from 'antd';
import {
...
const STANDARDEXTRACTION = 'standardExtraction';
const MODELEXTRACTION = 'modelExtraction';
const MYSQL = 'mysql';
const HIVE = 'hive';
const AS = 'as7';
const AR = 'AnyRobot';
...
const SourceModal = (props: any) => {
  const { operation, onSuccess, formInfo, selectKey, visible, handleCancel } = props;
  ...
  useEffect(() => {
    ...
    formRef.setFieldsValue({ ...formInfo, data_source: curSource, ds_path: curDSPath, ds_schema: curDSSchema });
    formRef.setFields([{ name: 'ds_password', touched: false }]);
  ...
  const onSourceChange = (value: any) => {
    ...
    formRef.resetFields(['ds_user', 'ds_password', 'ds_path', 'ds_address', 'ds_port', 'ds_auth']);
    formRef.setFieldsValue({
   ...
   * 下拉菜单收起控制
   */
  const onDropdownVisible = (value: any) => {
   ...
   */
  const passwordFocus = () => {
    if ((operation === 'edit' || operation === 'copy') && !formRef.isFieldTouched('ds_password')) {
      formRef.setFieldsValue({ ds_password: '' });
    }
  };
   ...
   */
  const goAuth = async () => {
    const ip = formRef.getFieldValue('ds_address') || '';
    const key = formRef.getFieldValue('ds_auth') || '';
    const ipError = formRef.getFieldError('ds_address') || [];

    const port = formRef.getFieldValue('ds_port') || '';
    const portError = formRef.getFieldError('ds_port') || [];
    ...
    if (ip && ipError.length <= 0 && port && portError.length <= 0) {
      const response: any = await servicesDataSource.asAuthGet(ip, port, key);

      const { res, Code } = response || {};

      if (res !== undefined && !Code) {
        formRef.setFieldsValue({ ds_auth: res.ds_auth });
        setAuthStatus(1);
        formRef.setFields([
          {
            name: 'ds_auth',
   ...
   */
  const showTestError = (errInfo: any) => {
      ...
      .then(async value => {
        ...
        if (value.data_source.includes(AS)) {
          ...
          body = {
            ...
            ds_password: formRef.isFieldTouched('ds_password') ? btoa(value.ds_password) : value.ds_password,
            ds_path: value.ds_path?.replace(/(^\s*)|(\s*$)/g, '') || '',
            vhost: value.vhost || '',
            queue: value.queue || '',
        ...
        const response: any = await servicesDataSource.sourceConnectTest(body);
  ...
  const onSave = async (e: any) => {
    ...
    formRef
      ...
      .then(async value => {
        ...
        const resTest: any = await servicesDataSource.sourceConnectTest(testBody);
   ...
   */
  const userFocus = () => {
    if (!formRef.isFieldTouched('ds_user')) {
      formRef.setFieldsValue({ ds_user: '' });
    }
  };
  ...
  return (
    ...
    >
      <div
        className="formRef-box"
        style={{
          height: sourceType === 'AnyRobot' ? 291 : 482,
          paddingBottom: sourceType === 'AnyRobot' ? 6 : undefined,
          marginBottom: sourceType !== 'AnyRobot' ? 28 : undefined
        }}
      >
        <KwScrollBar
          isShowX={false}
            ...
            >
                    ...
                    {allOptions &&
                      allOptions.map(item => (
                        <Option key={item.id} value={item.value} label={item.label}>
                          ...
                          {item.value.indexOf(ODBCPERFIX) > -1 ? <span style={{ marginRight: '3.5em' }}></span> : null}
                          {item.label}
                        </Option>
                      ))}
                  </Select>
              ...
              {sourceType.includes(AS) && (
                <div className="formRef-row">
                  ...
                  >
                    <Select
                      onChange={value => formRef.setFieldsValue({ extract_type: value })}
                      getPopupContainer={triggerNode => triggerNode.parentElement}
              ...
              <div className="formRef-row combo-row">
                {/* 地址 */}
                <FormItem
                  name="ds_address"
              ...
              {[MYSQL, HIVE, MQ, SQLSERVER, MYSQLODBC, KINGBASEES, KINGBASE, POSTGRESQL, CLICKHOUSE].includes(
                ...
                <div className="formRef-row combo-row">
                  ...
                  >
                    <Input autoComplete="off" onFocus={userFocus} placeholder={intl.get('datamanagement.pleaseUser')} />
                  </FormItem>
                    ...
                    >
                      {operation === 'edit' && navigator.userAgent.indexOf('Edge') > -1 ? (
                        <div className="disabled-input-out">
                          <div className="disabled-input-in">{formRef.getFieldValue('ds_path')}</div>
                        </div>
                      ) : (
                        <Input
                          disabled={operation === 'edit'}
              ...
              {/* rabbitmq的json模板 */}
              {sourceType === MQ && (
                <div className="formRef-row formRef-textarea">
