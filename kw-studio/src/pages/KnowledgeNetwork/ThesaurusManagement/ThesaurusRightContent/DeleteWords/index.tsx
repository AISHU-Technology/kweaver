    ...
    >
      <div className="delete-modal-title">
        ...
        <span className="title-text">
          {deleteType === 'one'
            ? intl.get('ThesaurusManage.delOneWordTitle')
            : intl.get('ThesaurusManage.delmoreTitle')}
        </span>
      </div>
      <div className="delete-modal-body">
        {deleteType === 'one' ? (
          <span>{intl.get('ThesaurusManage.delOneDes')}</span>
        ) : (
          <div>
            {intl.get('ThesaurusManage.delmoreDes').split('|')[0]}
            <span className="kw-c-error kw-ml-1 kw-mr-1">{deleteValue.length}</span>
            {intl.get('ThesaurusManage.delmoreDes').split('|')[1]}
          </div>
        )}
      </div>
