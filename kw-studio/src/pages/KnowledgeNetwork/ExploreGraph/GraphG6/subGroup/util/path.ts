...
import type { IPoint } from '@antv/g6';
 ...
 */
export const getControlPoint = (startPoint: IPoint, endPoint: IPoint, percent = 0, offset = 0): IPoint => {
  const point: IPoint = {
 ...
 * @return {Array} Path
 */
export const pointsToPolygon = (points: IPoint[] | ({ x: number; y: number } | string)[], z?: boolean): string => {
  const { length } = points;
  if (!length) {
    return '';
  }
