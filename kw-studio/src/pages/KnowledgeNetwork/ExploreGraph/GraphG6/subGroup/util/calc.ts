...
export const move = (
  ...
  point: Point,
  animate?: boolean,
  animateCfg: GraphAnimateConfig = { duration: 500 }
) => {
...
const lineIntersectPolygon = (lines: object, line: { from: Point; to: Point }) => {
...
export const isPolygonsIntersect = (points1: number[][], points2: number[][]): boolean => {
  ...
  const getBBox = (points: any[]): BBox => {
    const xArr = points.map((p: any[]) => p[0]);
    const yArr = points.map((p: any[]) => p[1]);
    return {
...
export const itemIntersectByLine = (item: Item, line: Line): [IPoint[], number] => {
  ...
  return [intersections as any, countIntersections];
};
...
export const isPointsOverlap = (p1: IPoint, p2: { x: any; y: any }, e = 1e-3) => {
 ...
 */
export const pointLineDistance = (line: [any, any, any, any], point: { x: any; y: any }) => {
