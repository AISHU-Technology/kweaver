...
import { Tabs, Button, Checkbox, Dropdown, Popconfirm } from 'antd';
...
export interface EntitySelectorProps {
  ...
  hideSearchBtn?: boolean; // 多个参数时, 悬停行不出现快捷查询按钮
...
const EntitySelector = (props: EntitySelectorProps) => {
  ...
  const { nodes, selectedNodes } = useMemo(() => {
    const nodes = canvasInstance?.apis?.getGraphData()?.nodes || [];
    let selectedNodes = [];
    try {
  ...
  useEffect(() => {
    ...
    const filterNodes = _.filter(nodes, n => _.includes(tags, n?.class));
    const input = !multiple ? filterNodes[0]?.id : _.map(filterNodes, n => n.id);
  ...
  useEffect(() => {
    ...
    if (_.isEmpty(nodes) && _.isEmpty(value)) {
      setSearchValue('');
      curInputValue.current = '';
      query('', nodes);
      return;
    }
    query(text, nodes);
  }, [JSON.stringify(value), multiple, nodes, tags]);
  ...
  const getListByCanvas = (keyword: string, nodeList: any[]) => {
    ...
    // 完全匹配靠前
    const sortedData = _.sortBy(result, item => {
   ...
   * 触发搜索
   */
  const handleSearch = (action: 'add' | 'cover', data: Record<string, any> | Record<string, any>[]) => {
    handleClose();
    if (_.isArray(data)) {
      const input = _.map(value, v => v.id);
      onSearch?.(action, input);
      return;
    }
          ...
          return (
            <div key={id} className={classNames('entity-row kw-align-center kw-pointer', { checked })}>
              ...
              <div className="e-name">
                <div className="kw-c-header kw-w-100 kw-ellipsis" title={text} onClick={() => onRowClick(item)}>
                  {text}
                </div>
              ...
              {!hideSearchBtn && (
                <>
                  <Button className="s-btn kw-ml-1" type="default" onClick={() => handleSearch('cover', item)}>
                    {intl.get('analysisService.coverBtn')}
                  </Button>
                  <Button className="s-btn kw-ml-2" type="primary" ghost onClick={() => handleSearch('add', item)}>
                    {intl.get('analysisService.addBtn')}
                  </Button>
                </>
              )}
            </div>
  ...
  return (
    <Dropdown
      overlayClassName={classNames(overlayClassName, 'canvas-entity-selector-overlay', {
        [dropdownSize]: true,
        ...
        <div className="dropdown-content" onMouseDown={onPreventMouseDown}>
          <Tabs activeKey={tabKey} onChange={setTabKey}>
            ...
            <Tabs.TabPane tab={intl.get('analysisService.canvasTab')} key={CANVAS} disabled={!nodes.length}>
              <div className="overlay-list" onScroll={onPreventMouseDown}>
                {canvasResult.length ? renderList(canvasResult) : <Empty />}
              </div>
            </Tabs.TabPane>
            <Tabs.TabPane tab={null} key={SELECTED}>
              <div className="overlay-list" onScroll={onPreventMouseDown}>
                {selectedResult.length ? renderList(selectedResult) : <Empty />}
              </div>
            </Tabs.TabPane>
          </Tabs>
          {multiple && (
          ...
          {multiple && (
            <div className="multiple-search-bar">
              <Button
                className="s-btn"
                type="default"
                disabled={!value?.length}
                onClick={() => handleSearch('cover', value!)}
              >
                {intl.get('analysisService.coverBtn')}
              </Button>
              <Button
                className="s-btn kw-ml-2"
                type="primary"
                ghost
                disabled={!value?.length}
                onClick={() => handleSearch('add', value!)}
              >
                {intl.get('analysisService.addBtn')}
              </Button>
            </div>
    ...
    >
      <div ref={setSelectorDOM} className={classNames(className, 'canvas-entity-selector')}>
        <InputSelector
