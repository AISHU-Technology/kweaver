...
import Canvas from '../../ConfigAndTest/Canvas';
import { DEFAULT_CANVAS } from '../../ConfigAndTest/enum';
import { generateCanvasConfig, isCheckedTool } from '@/pages/CognitiveService/AnalysisServiceTest/assistant';
...
const PC_ConfigModal = (props: PC_ConfigModalProps) => {
  const { className, zIndex = 10, visible, savedPCConfig, basicData, configInfo, onBack, onSave } = props;
  ...
  const onConfigChange = (data: any[]) => {
    ...
    canvasRef.current.faker = !config?.options?.click?.options?.information?.checked;
  ...
  return createPortal(
    <div
      className={classNames('pc_config-modal-root', className)}
      style={{ display: visible ? 'block' : 'none', zIndex }}
    >
      <div className="c-body">
        <div className="c-header kw-align-center">
          <Button onClick={handleGoBack} type="text" className="kw-pl-5">
            <LeftOutlined style={{ transform: 'translateY(1px)' }} />
            <span>{intl.get('global.back')}</span>
          </Button>
          <Format.Title className="t-name" strong={4}>
            {intl.get('analysisService.embedTitle')}
          </Format.Title>
        </div>
        <div className="c-main kw-flex">
          <div className="side-bar kw-flex-column kw-h-100" style={{ width: isExpand ? 440 : 56 }}>
            ...
            <div style={{ display: !isExpand ? 'none' : undefined }}>
              <div className="side-title kw-space-between">
            ...
            <div className="kw-flex-item-full-height" style={{ display: !isExpand ? 'none' : undefined }}>
              <EmbedConfig ref={PCRef} savedPCConfig={savedPCConfig} basicData={basicData} onChange={onConfigChange} />
            </div>
          </div>
          <div className="right-box kw-flex-column kw-flex-item-full-width kw-p-4">
            <div className="canvas-box kw-flex-item-full-height">
              ...
              <Canvas
                ...
                configMenu={{
                  click: canvasConfig?.click?.options || {},
                  hover: canvasConfig?.hover?.options || {},
            ...
            <div className="kw-center kw-pt-5 kw-pb-1">
              <Button type="default" style={{ marginRight: 6 }} onClick={onReset}>
                {intl.get('cognitiveSearch.default')}
              </Button>
              <Button type="primary" style={{ marginLeft: 6 }} onClick={handleSave}>
