...
import { Radio, Space, Checkbox, Input, message, Switch, Collapse } from 'antd';
...
import GRAPH_LAYOUT from '@/enums/graph_layout';
...
const defaultFeatures = {
  ...
  resultPanelDisplay: { visible: false, value: 'notDisplayResult' }
};
...
const EmbedConfig = (props: any, ref: any) => {
  ...
  const graphLayoutPattern = basicData?.graphLayoutPattern;
  ...
  const renderConfig = (data: any) => {
    const { key, deps, children, level, title, allowCheck, allowAdd, renderConfigs } = data;
    ...
    // 二级标题
    if (level === 'h2') {
      if (deps && graphLayoutPattern && !_.includes(deps, graphLayoutPattern)) return null;
      if (key === 'toolbar_algorithm' && graphLayoutPattern === GRAPH_LAYOUT.TREE) {
        return null;
      }
  ...
  const renderChildren = (parentKey: string, allowAdd?: boolean) => {
    ...
    if (parentKey === 'toolbar_search' && graphLayoutPattern === GRAPH_LAYOUT.TREE) {
      const temp = data.children.filter((item: any) =>
        ['toolbar_search_search', 'toolbar_search_neighbors'].includes(item.key)
      );
      data.children = temp;
    }
  ...
  const renderChildrenItem = (parentKey: string) => {
    ...
    const list = _.filter(data.children, item => {
      if (item.deps && graphLayoutPattern && !_.includes(item.deps, graphLayoutPattern)) return false;
      return true;
    });
    ...
    return _.map(list, item => {
      ...
      let label = showTitle(key);
      if (alias || graphLayoutPattern === GRAPH_LAYOUT.TREE) label = showTitle(key, graphLayoutPattern);
      if (isCustom) label = alias;
      ...
      return (
        <div key={key} className="children-item kw-align-center">
          ...
          />
          <span className="i-name kw-c-text kw-ellipsis" title={label}>
            {label}
          </span>
          <span
  ...
  return (
    <div className="analysis-service-embed-config">
      <div className="scroll-wrap">
        <div className="config-parent-item">
          <div className="p-title kw-space-between kw-mt-4 kw-c-header">
            <span>{intl.get('analysisService.welcomeT')}</span>
            <Switch className="kw-ml-5" checked={features.welcomeMessage.visible} onChange={onWelcomeSwitchChange} />
          </div>
          {features.welcomeMessage.visible && (
            <>
              <Input.TextArea
                className={classNames({ 'error-border': features.welcomeMessage.error })}
                placeholder={intl.get('analysisService.welcomePlace')}
                rows={3}
                value={features.welcomeMessage.content}
                onChange={onWelcomeChange}
              />
              <div className="error-msg">{features.welcomeMessage.error}</div>
            </>
          )}
        </div>
        ...
        <Collapse className="config-Collapse" ghost>
          <Collapse.Panel
            key={'paramsTool'}
            header={
              <div className="p-title kw-c-header">
                {intl.get('analysisService.queryT')}
                <ExplainTip arrowPointAtCenter title={intl.get('analysisService.queryTip')} />
              </div>
            }
          >
            <Radio.Group
              value={features.resultPanelDisplay.value}
              onChange={e => {
                const value = e.target.value;
                dispatchFeatures({ resultPanelDisplay: { value } });
              }}
            >
              <Space className="kw-pl-6" direction="vertical">
