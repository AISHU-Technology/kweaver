 ...
 * @param key 配置的key
 */
export const showTitle = (key: string, pattern = 'common') => {
  let _key = key;
  // 兼容代码
  ...
  if (pattern === 'tree' && _key === 'toolbar_search_search') _key = 'toolbar_canvas_start';
...
const defaultConfig = [
    ...
    children: [
      {
        ...
        children: [
          ...
          { key: 'sliced', name: '图切片', deps: [GRAPH_LAYOUT_PATTERN.COMMON] },
          { key: 'simpleSearch', name: '搜索' },
          { key: 'simpleSql', name: '语句查询', deps: [GRAPH_LAYOUT_PATTERN.COMMON] },
          { key: 'simpleNeighbors', name: '邻居查询' },
          { key: 'simplePath', name: '路径查询', deps: [GRAPH_LAYOUT_PATTERN.COMMON] }
        ]
      },
      {
        key: 'search',
    ...
    children: [
      {
        ...
        children: [
          ...
          { key: 'subgraph', name: '选中子图', deps: [GRAPH_LAYOUT_PATTERN.COMMON] },
          { key: 'sliced', name: '图切片', deps: [GRAPH_LAYOUT_PATTERN.COMMON] }
        ]
      }
    ]
    ...
    children: [
      {
        ...
        children: [
          { key: 'style', name: '外观样式' },
          { key: 'fixed', name: '固定当前位置', deps: [GRAPH_LAYOUT_PATTERN.COMMON] },
          { key: 'invert', name: '反选' },
          { key: 'remove', name: '移除' },
          { key: 'hide', name: '隐藏' },
          { key: 'selectSame', name: '选中相同类' },
      ...
      },
      {
        ...
        children: [
          { key: 'neighbors', name: '邻居查询' },
          { key: 'path', name: '路径探索', deps: [GRAPH_LAYOUT_PATTERN.COMMON] }
        ]
      }
    ]
  },
  ...
  {
    key: 'click',
    children: [{ key: 'information', name: '显示基本信息侧边栏', visible: false }]
  },
  {
    key: 'hover',
    children: [{ key: 'tooltip', name: '显示基本信息卡片', visible: false }]
  },
  // 功能设置
  {
    key: 'features',
