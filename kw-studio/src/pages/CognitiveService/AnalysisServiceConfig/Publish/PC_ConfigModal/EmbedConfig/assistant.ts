...
export const generateConfig = (
    ...
    return _.map(origin, item => {
      ...
      if (_.split(item.key, '_').length === 2) {
        if (_.isEmpty(item.children)) {
          thisItem.key = _.split(item.key, '_').pop();
          thisItem.pKey = _.split(item.pKey, '_').pop();
          thisItem.checked = !!keyMap[item.key];
        } else {
          thisItem.children = _.map(item.children, c => {
  ...
  const featuresConfig = {
    ...
    children: [
      ...
      {
        key: 'resultPanelDisplay',
        value: features.resultPanelDisplay.value,
        visible: features.resultPanelDisplay.visible
      }
    ]
  };
...
export const mergeSavedConfig = (savedConfig: any[]) => {
  ...
  const loopMerge = (arr: any[]) => {
    return _.map(arr, item => {
      ...
      if (_.split(item.key, '_').length === 2 && !_.isEmpty(item.children)) {
  ...
  if (savedConfigMap.features_resultPanelDisplay) {
    defaultFeatures.resultPanelDisplay.visible = savedConfigMap.features_resultPanelDisplay.visible;
    defaultFeatures.resultPanelDisplay.value = savedConfigMap.features_resultPanelDisplay.value;
  }
