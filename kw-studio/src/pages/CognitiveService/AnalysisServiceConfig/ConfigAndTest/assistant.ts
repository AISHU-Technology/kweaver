 ...
 * @param canvas 画布实例
 */
export const generateCanvasData = (canvas: any, rules?: any) => {
  if (!canvas.graph) return { canvas_body: '', canvas_config: JSON.stringify({ key: GRAPH_LAYOUT.FREE }) };
  const { graphData, graphStyle, graph, graphConfig, layoutConfig, sliced } = canvas;
  ...
  const canvas_config = layoutConfig?.key ? layoutConfig : DEFAULT_CANVAS.layoutConfig;
 ...
 * 应用配置时更新参数的坐标和输入值类型
 */
export const updatePosition = (oldData: any[], newData: any[], ontoEntity?: any[]) => {
  ...
  return _.map(oldData, item => {
    if (map[item._id]) {
      ...
      if (item.param_type === 'entity') {
        ...
        item.entity_classes = item?.entity_classes ? item?.entity_classes : _.map(ontoEntity, e => e?.name);
      }
