...
const constructGraphData = (_data: any, option?: OptionType): any => {
  ...
  const nodes = _.map(nodeWithSize || [], (item: NodeType) => {
    const { vid: uid, alias, color, icon, x, y, default_property } = item;
    const label = default_property.v.length < 20 ? default_property.v : `${default_property.v.substring(0, 17)}...`;
    ...
    return {
      ...
      label: `${label}`,
      icon,
      fx: isDef(x) ? x : undefined,
      fy: isDef(y) ? y : undefined,
      ...(extendOption || {}),
      ...(option?.extendNode || {}),
      size: 40,
      _sourceData: item
  ...
  const edges = _.map(data.edges || [], (item: EdgeType) => {
    const { edge_id: uid, alias, color, src_id, dst_id } = item;
    const source = src_id;
    const target = dst_id;
