...
const PromptHome = (props: any) => {
  ...
  const operateType = useRef<any>(''); // 操作方式
  ...
  return (
    <div className={classNames(className, 'prompt-home-root kw-flex kw-h-100')}>
      <ProjectList
        ref={listRef}
        data={projectList}
            ...
            <NoDataBox
              ...
              desc={
                <>
                  ...
                  >
                    {intl.get(`prompt.${projectState.total ? 'createGroupTip' : 'createProjectTip'}`).split('|')[1]}
                  </span>
                  {intl.get(`prompt.${projectState.total ? 'createGroupTip' : 'createProjectTip'}`).split('|')[2]}
                </>
              }
            />
          </div>
