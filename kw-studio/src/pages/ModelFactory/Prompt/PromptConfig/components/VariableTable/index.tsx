...
const VariableTable = (props: VariableTableProps) => {
  ...
  const tip = (
    <>
      {intl.get('prompt.varWarning1')}
      &nbsp;
      <IconFont type="icon-tongbubianliang" />
      &nbsp;
      {intl.get('prompt.var')}
  ...
  return (
    <div className={classNames(className, 'mf-prompt-variable-table')}>
      <div className="table-title kw-space-between">
        <div>
          <IconFont type="icon-bianliang" className="kw-c-warning" style={{ fontSize: 16 }} />
          <Format.Title className="kw-ml-2">{intl.get('prompt.var')}</Format.Title>
          <ExplainTip title={tip} />
        </div>
        {!disabled && (
          <div className="kw-c-text-link" onClick={clickAdd}>
            <IconFont type="icon-Add" />
            <span className="kw-ml-2">{intl.get('global.add')}</span>
          </div>
        )}
      </div>
      {variables.length ? (
        <div className="variable-table-box">
          <div className="var-th kw-align-center">
            ...
            <div className="th-op">{intl.get('global.operation')}</div>
          </div>
        ...
        <div className="kw-mt-2 kw-mb-2 kw-c-text-lower" style={{ fontSize: 12 }}>
          {tip}
        </div>
      )}
