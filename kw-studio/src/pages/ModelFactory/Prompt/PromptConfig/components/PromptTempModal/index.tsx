...
const PromptTempModal = (props: PromptTempModalProps) => {
  const { className, visible, templates, onOk, onCancel, promptManageList } = props;
  ...
  const getData: Function = async (node: any) => {
    try {
      ...
      if (res) {
        ...
        const { prompt_type } = getParam(['prompt_type']);
        const filterData = _.filter(_.cloneDeep(data), (item: any) => item?.prompt_type === String(prompt_type));
...
export default (props: Omit<PromptTempModalProps, 'templates'>) => {
  ...
  return props.visible ? (
    <PromptTempModal {...props} templates={templates} promptManageList={promptManageList} />
  ) : null;
};
