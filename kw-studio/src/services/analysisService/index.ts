...
const analysisServiceGet = async (id: string | number, type?: string) => {
  ...
  if (isIframe()) {
    return await openApiService.axiosGet(`${OPEN_API.openAnalysisServiceGet}/${id}`);
  }
...
const analysisServiceTest = async (body: any) => {
  if (isIframe()) {
    return await openApiService.axiosPost(OPEN_API.openAnalysisServiceTest, body, { timeout: 600000 });
  }
