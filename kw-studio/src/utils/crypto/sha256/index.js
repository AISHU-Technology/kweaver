...
/**
 * openApi 加密appkey
 * @param {string} appid AD appid
 * @param {string} timestamp 时间戳 精确到秒
 * @param {string} params 请求参数
 * @returns {string} appkey
 */
export function encodeAppKey(appid, timestamp, params) {
  //时间戳加密
  ...
  const message = Hex.stringify(sha256timestamp) + Hex.stringify(sha256paramString);

  // Python: hmac.new(self.appid.encode("utf-8"), message, digestmod=sha256).digest()
  let shaString = hmacSHA256(message, appid);
  // Python: result = base64.b64encode(shaString.hex().encode("utf-8"))
  shaString = Base64.stringify(Utf8.parse(shaString));
