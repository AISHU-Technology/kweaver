...
installed_services="${mariadb} ${redis} ${mongodb}"
...
for ((;;));do
  ...
  if [ "$installed_str" == "" ];then
    echo -e "\033[31;1m\nNo KWeaver persistence component service is running in the current cluster, exit the current program.\033[0m"
    exit
  else
    echo -e "\033[32;1mThe currently installed KWeaver persistence components are as follows:\n${installed_str}\n\033[0m"
  fi
  if [ "$errorMsg" != "" ];then
    echo -e ${errorMsg}
  fi
  echo -en "\033[34;1mPlease enter the KWeaver persistence component service tag to delete([0-${idQueue[-1]}], 0 means delete all KWeaver persistence services)\033[0m: "
  read num
  ...
  if [ $? -ne 0 ] || [ "$num" == "" ];then
    ...
    unset arr[$start]
    if [ ${#arr[@]} -ne 0 ];then
      echo -en "\033[36;1m\nWhether to continue uninstalling other KWeaver persistence components(Y/y)/(Other options or press Enter to exit the current uninstaller)\033[0m: "
      read decision
