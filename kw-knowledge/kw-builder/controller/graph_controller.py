...
@graph_controller_app.route('/add', methods=["post"], strict_slashes=False)
def graphopt():
    '''
    GET请求：返回所有图谱信息
...
@graph_controller_app.route('/graph_db', methods=["get"], strict_slashes=False)
def getgraphdb():
    '''
...
@graph_controller_app.route('/<graph_id>', methods=["get"], strict_slashes=False)
def getgraphbyid(graph_id):
    host_url = getHostUrl()
...
@graph_controller_app.route('/save_no_check', methods=["post"], strict_slashes=False)
def savenocheck():
    try:
        host_url = getHostUrl()
        param_code, params_json, param_message = commonutil.getMethodParam()
...
@graph_controller_app.route('/ds/list', methods=["get"], strict_slashes=False)
def getdsbygraphids():
    '''
    根据图谱id返回数据源列表
    '''
...
@graph_controller_app.route('/info/simple', methods=["post"], strict_slashes=False)
...
# 图谱批量删除
@graph_controller_app.route('/delete', methods=["POST"], strict_slashes=False)
...
def graphDeleteByIds():
    ...
    res = knw_dao.check_knw_graph(params_json["knw_id"], graph_ids)
    if len(res) != len(graph_ids):
        code = codes.Builder_GraphController_GraphDeleteByids_GraphIdNotExist

        return Gview2.TErrorreturn(code, graph_id=graph_ids), 500
    # 统计运行状态的
    res_mess, res_code = graph_Service.getStatusByIds(graph_ids)
    if res_code != 0:
    ...
    # 统计不存在的id
    res, code = graph_Service.getNoExistIds(graph_ids)
    if code != 0:
        code = codes.Builder_GraphController_GraphDeleteByIds_UnknownError

        return Gview2.TErrorreturn(code, description=res["message"], cause=res["cause"]), 500
    noExist = res["noExist"]
    normal = list(set(graph_ids) - set(noExist) - set(runs))
...
@graph_controller_app.route('/subgraph/add', methods=['post'], strict_slashes=False)
...
@graph_controller_app.route('/subgraph/save_no_check/<graph_id>', methods=['post'], strict_slashes=False)
...
@graph_controller_app.route('/subgraph/list', methods=['get'], strict_slashes=False)
...
@graph_controller_app.route('/edit/<graphid>', methods=["post"], strict_slashes=False)
def graph(graphid):
    '''
    根据图谱id编辑图谱信息
