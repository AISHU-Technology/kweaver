    ...
    if method == "POST":
        ...
        if param_code == 0:
            ...
            operation_log = log_oper.get_operation_log(user_name, log_oper.UPDATE, graph_id, params_json,
                                                       "修改了图谱{id=%s}的定时构建任务{id=%s}" % (graph_id, task_id))
            Logger.log_info(operation_log)
            obj = {"state": "success"}
    ...
    if method == "POST":
        ...
        if param_code == 0:
            ...
            operation_log = log_oper.get_operation_log(user_name, log_oper.DELETE, graph_id, params_json,
                                                       "删除了图谱{id=%s}的定时构建任务{id=%s}" % (graph_id, task_ids))
            Logger.log_info(operation_log)
            obj = {"state": "success"}
        ...
        if param_code == 0:
            ...
            if code != CommonResponseStatus.SUCCESS.value:
                ...
                return Gview.TErrorreturn(code, desc, solution, detail,
                                          error_link), CommonResponseStatus.SERVER_ERROR.value
            user_name = request.headers.get("username")
            operation_log = log_oper.get_operation_log(user_name, log_oper.UPDATE, graph_id, {},
                                                       "设置了图谱{id=%s}的定时构建任务{id=%s}开关为%s" % (graph_id, task_id, enabled))
            Logger.log_info(operation_log)
            obj = {"state": "success"}
