...
from utils.beat_orm import PeriodicTask, PeriodicTaskChanged, CrontabSchedule
...
class ModelEntry(ScheduleEntry):
    ...
    def __init__(self, model, Session, app=None, **kw):
        ...
        self.kw_session = kw.get('session')
        ...
        try:
            ...
            Logger.log_debug('schedule: {}'.format(self.schedule))
        except Exception as e:
            error_log = log_oper.get_error_log(repr(e), sys._getframe(), traceback.format_exc())
            Logger.log_error(error_log)
            error_log = log_oper.get_error_log('Disabling schedule {} that was removed from database'.format(self.task_id), sys._getframe(), traceback.format_exc())
            Logger.log_error(error_log)
            self._disable(model)
        ...
        try:
            ...
            error_log = log_oper.get_error_log('Removing schedule {} for argument deseralization error: {}'.format(self.task_id, exc), 
                                             sys._getframe(), traceback.format_exc())
            Logger.log_error(error_log)
            self._disable(model)

        self.options = {}
        for option in ['queue', 'f_exchange', 'routing_key', 'expires',
                       'priority']:
    ...
    def _disable(self, model):
        ...
        if self.kw_session:
            self.kw_session.add(model)
            self.kw_session.commit()
        else:
    ...
    @classmethod
    def from_entry(cls, task_id, Session, app=None, **entry):
        """
        ...
        **entry sample:
        ...
        with session_cleanup(session):
            periodic_task = session.query(
                PeriodicTask).filter_by(task_id=task_id).first()
            if not periodic_task:
                periodic_task = PeriodicTask(task_id=task_id)
            ...
            try:
                ...
                error_log = log_oper.get_error_log(str(exc), sys._getframe(), traceback.format_exc())
                Logger.log_error(error_log)
                session.rollback()
    ...
    @classmethod
    def _unpack_options(cls, queue=None, f_exchange=None, routing_key=None,
    ...
    def __repr__(self):
        return '<ModelEntry: {0} {1}(*{2}, **{3}) {4}>'.format(
            safe_str(self.task_id), self.task, safe_repr(self.args),
...
class DatabaseScheduler(Scheduler):
    ...
    def setup_schedule(self):
        """override"""
        Logger.log_info('setup_schedule')
    ...
    def all_as_schedule(self):
        ...
        session = self.Session()
        with session_cleanup(session):
            Logger.log_debug('DatabaseScheduler: Fetching database schedule')
            # get all enabled PeriodicTask
            models = session.query(self.Model).filter_by(enabled=True).all()
            s = {}
            for model in models:
                try:
                    s[model.task_id] = self.Entry(model,
                                                  app=self.app,
                                                  Session=self.Session,
                                                  session=session)
                except ValueError:
    ...
    def reserve(self, entry):
        ...
        # Need to store entry by task_id, because the entry may change
        # in the mean time.
        self._dirty.add(new_entry.task_id)
        return new_entry

    def sync(self):
        """override"""
        Logger.log_info('Writing entries...')
        _tried = set()
        _failed = set()
        try:
            while self._dirty:
                task_id = self._dirty.pop()
                try:
                    self.schedule[task_id].save()  # save to database
                    Logger.log_debug('{task_id} save to database'.format(task_id=task_id))
                    _tried.add(task_id)
            ...
            error_log = log_oper.get_error_log('Database error while sync: {}'.format(exc), sys._getframe(), traceback.format_exc())
            Logger.log_error(error_log)
        except Exception as exc:
            if 'last_run_at' not in str(exc):
                error_log = log_oper.get_error_log(str(exc), sys._getframe(), traceback.format_exc())
                Logger.log_error(error_log)
        finally:
    ...
    def update_from_dict(self, mapping):
        s = {}
        for task_id, entry_fields in mapping.items():
            # {'task': 'celery.backend_cleanup',
            #  'schedule': schedules.crontab('0', '4', '*'),
            #  'options': {'expires': 43200}}
            try:
                entry = self.Entry.from_entry(
                    task_id, Session=self.Session, app=self.app,
                    **entry_fields)
                if entry.model.enabled:
                    s[task_id] = entry
            except Exception as exc:
                error_log = log_oper.get_error_log("{} {} {} {}".format(ADD_ENTRY_ERROR, task_id, exc, entry_fields), sys._getframe(), traceback.format_exc())
                Logger.log_error(error_log)
    ...
    def schedule(self):
        ...
        if self._initial_read:
            Logger.log_debug('DatabaseScheduler: initial read')
            initial = update = True
            self._initial_read = False
        elif self.schedule_changed():
            # when you updated the `PeriodicTasks` model's `last_update` field
            Logger.log_info('DatabaseScheduler: Schedule changed.')
            update = True
        ...
        if update:
            ...
            Logger.log_debug('Current schedule:\n{}'.format('\n'.join(
