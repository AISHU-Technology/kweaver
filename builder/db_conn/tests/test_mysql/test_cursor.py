...
class CursorTest(PyMySQLTestCase):
    ...
    @using_mysql
    def test_cleanup_rows_unbuffered(self, db):
        conn = db.connection
        cursor = conn.cursor(pymysql.cursors.SSCursor)

        res = cursor.execute("select * from test as t1, test as t2")
        print(res)
    ...
    @using_mysql
    def test_cleanup_rows_buffered(self, db):
        conn = db.connection
