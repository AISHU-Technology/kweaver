...
# 建立SQLAlchemy MySQL的连接
class DBSQLAlchemyLib(object):
    ...
    def __connect(self, db_config, retry_nums=0, retry_interval=1):
        ...
        """
        try:
            g_mysql_url = 'mysql+pymysql://%s:%s@%s:%d/%s' % (db_config.user, db_config.password, db_config.host,
                                                              db_config.port, db_config.database)

            engine = create_engine(g_mysql_url, **self._pymysql_param)
