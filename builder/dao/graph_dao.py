...
class GraphDao():
    ...
    def updategraph(self, id, params_json, otl_id, connection, cursor):
        ...
        sql = """UPDATE knowledge_graph SET update_user=%s, update_time=%s, KG_name=%s,KDB_ip=%s  where KG_config_id =%s"""
    ...
    def insertgraph(self, params_json, ret, connection, cursor):
        ...
        sql = """INSERT INTO knowledge_graph (create_user,update_user, create_time, update_time, graph_update_time,KG_config_id, KG_name, 
         KDB_ip,KDB_name,status,hlStart,hlEnd) VALUES(%s,%s, %s, %s,%s, %s, %s,%s, %s, %s, %s, %s)"""
        Logger.log_info(sql)
    ...
    def insertData(self, params_json, connection, cursor):
        ...
        sql = """INSERT INTO graph_config_table (create_user,update_user,create_time, update_time, graph_name,graph_status, 
    ...
    def input_data(self, knowledge_network, graph_config, knowledge_graph, ontology, search_configs, isUpdate,
        ...
        if not knw_id:
            if knowledge_network["is_update"]:
                ...
                knowledge_network_value_list.append(knowledge_network["creator_id"])
                knowledge_network_value_list.append(knowledge_network["final_operator"])
        ...
        if isUpdate:
            ...
            knowledge_graph_value_list.append(knowledge_graph["create_user"])
            knowledge_graph_value_list.append(knowledge_graph["create_time"])
            knowledge_graph_value_list.append(knowledge_graph["update_user"])
            ...
            for search_config in search_configs:
                if search_config["is_update"]:
                    ...
                    search_config_value_list.append(search_config["create_user"])
                    search_config_value_list.append(search_config["update_user"])
    ...
    def savenocheck(self, graph_id, params_json, connection, cursor):
        ...
        values_list.append(arrow.now().format('YYYY-MM-DD HH:mm:ss'))
        # values_list.append("edit")
        # sql = """UPDATE graph_config_table SET creat_user=%s, update_time=%s,graph_status=%s """
        sql = """UPDATE graph_config_table SET create_user=%s, update_time=%s """
        ...
        #     sql = sql.replace('%s', '"'+str(values)+'"', 1)
        # sql = sql % ('"' + values_list[0] + '"',
        #              '"' + values_list[1] + '"',
        #              '"' + values_list[2] + '"',
        #              '"' + values_list[3] + '"', values_list[4])
        # sql = sql % (values_list)
        Logger.log_info(sql)
        # cursor.execute(sql)
        cursor.execute(sql, values_list)
    ...
    def update_otl_temp(self, grapid, connection, cursor):
        ...
        sql = """UPDATE graph_config_table SET update_user=%s, update_time=%s,graph_otl_temp=%s  where id = %s"""
    ...
    def update(self, id, params_json, otl_id, connection, cursor):
        ...
        if graph_step == "graph_baseInfo":
            ...
            sql = f"""UPDATE graph_config_table SET update_user=%s, update_time=%s, graph_name=%s, {graph_step}=%s, 
    ...
    def delete_record(self, graph_id, connection, cursor):
        ...
        sql3 = """DELETE FROM account_manager_graph WHERE manager_graph_id IN (SELECT id FROM manager_graph WHERE kg_id = (%s) AND type = 3)""" % str(
            graph_id)
        sql4 = """DELETE FROM manager_graph WHERE kg_id = (%s) AND type = 3""" % str(graph_id)
        cursor.execute(sql1)
        cursor.execute(sql2)
        cursor.execute(sql3)
        cursor.execute(sql4)
    ...
    def deleteGraphByIds(self, graphids, connection, cursor):
        ...
        # sql4 = """DELETE FROM account_manager_graph WHERE manager_graph_id IN (SELECT id FROM manager_graph WHERE kg_id in ({}) AND type = 3)""".format(",".join(map(str, graphids)))
        # sql5 = """DELETE FROM manager_graph WHERE kg_id in ({}) AND type = 3""".format(",".join(map(str, graphids)))
        # 删除图谱所属的定时任务
    ...
    # 根据appid获取uuid
    @connect_execute_close_db
    def getuuid(self, appid, connection, cursor):
        sql = """SELECT uuid FROM account WHERE app_id={}""".format('"' + str(appid) + '"')
        print("sql: ", sql)
        df = pd.read_sql(sql, connection)
        return df
    ...
    # 根据uuid获取账户信息
    @connect_execute_close_db
    def get_account_by_uuid(self, uuid, connection, cursor):
        sql = """SELECT * FROM account WHERE uuid={}""".format('"' + str(uuid) + '"')
        print("sql: ", sql)
        df = pd.read_sql(sql, connection)
        return df
    ...
    def select_timed_page(self, graph_id, order_type, page, size, connection, cursor):
        sql = f"""select t.task_id,t.task_type,t.cycle,t.date_time as datetime,t.date_list,t.enabled,
                 ...
                 ifnull(a.name,"") as update_user,ifnull(a.email,"") as update_user_email from timer_task t left join
                 account a on t.update_user = a.uuid where t.graph_id={graph_id} order by t.create_time {order_type}
