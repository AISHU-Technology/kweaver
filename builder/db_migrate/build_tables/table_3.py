...
class KnowledgeGraph(Base):
    ...
    KDB_name_temp = Column(String(250))
...
class GraphTaskTable(Base):
    ...
    task_type = Column(String(50), nullable=True)
    subgraph_ids = Column(LONGTEXT, server_default='[0]')
    current_subgraph_id = Column(Integer, server_default='-1')
    failed_subgraph_ids = Column(LONGTEXT)
    stop_subgraph_ids = Column(LONGTEXT)
    write_mode = Column(String(50), server_default='skip')
    successful_subgraph = Column(LONGTEXT)
...
class GraphTaskHistoryTable(Base):
    ...
    subgraph_id = Column(Integer, server_default='0')
...
class SubgraphConfig(Base):
    __tablename__ = "subgraph_config"
    __table_args__ = {
        'mysql_charset': 'utf8'
    }
    id = Column(Integer, autoincrement=True, primary_key=True)
    ontology_id = Column(Integer, nullable=False)
