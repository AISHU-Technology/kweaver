...
class KnowledgeNetwork(Base):
    ...
    final_operator = Column(String(50), nullable=True)
...
def init_datatable():
    ...
    orient_port = os.getenv("ORIENTPORT")
    ...
    orient_passwd = base64.b64encode(orient_passwd.encode("utf-8"))
    now = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if not res:
        obj = GraphDB(ip=orient_ip,
                      port=orient_port,
                      user=orient_user,
                      password=orient_passwd,
                      version=3,
                      type="orientdb",
                      db_user=orient_user,
                      db_ps=orient_passwd,
    ...
    opensearch_passwd = base64.b64encode(os.getenv('OPENSEARCH_PASS').encode("utf-8"))
    res_os = session.query(FullTextEngine).filter(FullTextEngine.ip == opensearch_ip).first()
    if not res_os:
        obj_os = FullTextEngine(ip=opensearch_ip,
        ...
        session.commit()
    session.close()
    Logger.log_info("insert opensearch information success")
