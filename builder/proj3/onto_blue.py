    ...
    try:
        ...
        if retcode != 200:
            ...
            if status == False:
                ...
                if len(file_list) > 1:
                    ...
                    if name_code != 200:
                        return jsonify({'res': {"cause": task_name['cause'], "code": 500001,
                                                "message": "get multi task name wrong"}, "code": 500})
                else:
                    one = file_list[0]
                ...
                if add_code != 200:
                    return jsonify({'res': {"cause": add_res['cause'], "code": 500001, "message": "task start wrong"},
                                    "code": 500})
                ...
                start_code, celery_task_id = task_service.start_otl_task(new_params_json, task_id)
                if start_code == 200:
...
def get_all_otl_task():
    ...
    if len(df) == 0:
        return jsonify(
            {"res": {"cause": "ontology_id doesn`t exist!", "code": 500025, "message": "can`t get task information"},
             "code": 500})
