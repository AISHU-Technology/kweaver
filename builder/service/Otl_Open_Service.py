...
class OtlOpenService():
	...
	def getGraphById(self, graph_id):
		...
		otl_ids = []
		try:
			df = otlopendao.getbygraphid(graph_id)
			# df = graph_dao.getOtlBygraphId(graph_id)
			# df = graph_dao.getOtlById(graph_id)
			rec_dict = df.to_dict('records')
			df_otl = df.to_dict()["graph_otl"]
			for k, v in df_otl.items():
				otl_ids.extend(eval(v))
			...
			if len(rec_dict) == 0 or len(otl_ids) == 0:
				...
				obj = self.errorobj(CommonResponseStatus.OTL_OPEN_GRAPH_NOT_EXIST.value,
									...
									"graph not exist or otl not exist!", "")
				Logger.log_error(" graph {} not exist or otl not exist!".format(graph_id))
			else:
				# 校验图数据库是否存在、校验图数据库DB名是否存在
				rec_dict = rec_dict[0]
				DB_ERROR = False
				dbname_exits_flag = False
				res = graph_dao.getkgdbipnamebyid(graph_id)
				db_dict = res.to_dict("records")
				task_status = rec_dict["task_status"]
				if len(db_dict) == 0:
					Logger.log_error("graph data not exist!")  # 无数据
					DB_ERROR = True
				else:
					try:
						db_dict = db_dict[0]
						graphdb_id = eval(rec_dict['graph_baseInfo'])[0]['graph_db_id']
						graphdb = GraphDB(graphdb_id)
						graph_DBName = db_dict["KDB_name"]
						databases = graphdb.get_list()
	...
	def updateotlschema(self, params_json, df, graph_id):
		"""更新本体schema
		Args:
			params_json: 传入参数
			df: 图谱配置信息
		...
		ret_code, obj = self.generateorientsql(operationtype, datatype, classname, df, graph_id=graph_id, edgein=edgein,
											   edgeout=edgeout,
	...
	def generateorientsql(self, operationtype, datatype, classname, df, **kw):
		'''本体编辑和图谱schema修改 具体实现过程'''
		ret_code = CommonResponseStatus.SUCCESS.value
		NAME_EXIST_ERROR = CommonResponseStatus.OTL_OPEN_NAME_EXIST.value
		obj = {}
		graph_id = int(df.iloc[0]['id'])
		db = eval(df.iloc[0]['graph_baseInfo'])[0]['graph_DBName']
		graphdb_id = eval(df.iloc[0]['graph_baseInfo'])[0]['graph_db_id']
		graphdb = GraphDB(graphdb_id)
		try:
			orient_sql_list = []  # 需要执行的不是一个sql；默认创建name 属性，使用事务的方式创建
			if operationtype == "add":
				if datatype == "entity":
					schema_entity_old, schema_edge_old = self.getoldotl(df)
					...
					code, res = graphdb.create_class(db, classname, otl_pro=['name'],
													 en_pro_dict={classname: {'name': 'string'}})
					if code != 200:
						desc = errdetail = solu = ''
					...
					# 修改本体
					otl_dict, otlid = self.getotldict(df, classname)
					params_json_new = {}
					...
					for i in range(len(otl_old_en_ed)):
						if classname == otl_old_en_ed[i]["name"]:
							...
							class_properties.append([str(kw["propertyname"]), str(kw["propertytype"])])
				...
				if datatype == "entity" or datatype == "edge":
					...
					if graphdb.type != 'orientdb':
						return 500, self.errorobj(500001, 'nebula does not support alter class name.',
					...
					if classname in schema_old_en_ed:
						...
						return ret_code, obj
					if graphdb.type != 'orientdb':
						return 500, self.errorobj(500001, 'nebula does not support alter property name.',
					...
					for i in range(len(otl_old_en_ed)):
						if classname == otl_old_en_ed[i]["name"]:
							...
							for ptindex in range(len(class_properties)):
								...
								if pro_type[0] == kw["oldname"]:
									if kw["altertype"] == "type":
										pro_type[1] = kw["propertytype"]
									if kw["altertype"] == "name":
										pro_type[0] = kw["propertyname"]
	...
	def getotldict(self, df, classname):
		"""
		Args:
			df: 图谱配置信息
		Returns:
		...
		otlid_list = eval(graph_otl)
		otlid = otlid_list[0]  # 通过本体ID获取本体
		# 本体
		if graph_otl is not None and len(eval(graph_otl)) > 0:
			otl_df = otl_dao.getbyid(eval(graph_otl)[0])
			otl_dict = otl_df.to_dict('records')
			return otl_dict, otlid
		return {}, otlid

	def getoldotl(self, df):
		"""获取旧的本体信息
		Args:
			df: 图谱配置信息
		Returns:
